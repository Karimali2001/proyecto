...
class InitState(State):
    def process(self) -> None:
        ...
        from .run_tracking_state import RunTracking
        ...
        )  # Use known args to avoid conflict with main if any

        # 2. Initialize Hailo Driver
        try:
            self.context.hailo_driver = HailoDriver(
                args.model, args.labels, args.score_thresh
            )
        ...
        try:
            ...
            if self.context.camera_driver is None:
        ...
        # 4. Initialize Output Audio Driver
